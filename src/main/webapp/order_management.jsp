<%@ page import="com.airline.bean.Order" %>
        ...
        <div class="container">
            ...
            <div class="row">
                <table class="table table-striped">
                    ...
                    <thead>
                    <tbody>
                    <c:forEach items="${order_list}" var="order" varStatus="id">
                        <tr>
                            <td>${order.orderid}</td>
                            <td>Paypal</td>
                            <td>${order.status}</td>
...
<script type="text/javascript">
    ...
    $(".btn-primary").click(function () {
        var order_id = $(this).parent().siblings().first();
        var status = $(this).parent().siblings().eq(2)
        // console.log($(order_id).text(), $(status).text());

        if ($(status).text() != "Paid") {
            alert("Cannot refund an unpaid order!");
        } else {
            $.ajax({
                type: "GET",
                url: "${APP_PATH}/refundOrder.do",
                data: {
